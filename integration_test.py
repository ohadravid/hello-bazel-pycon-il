...
def OCIImageContainer(image):
    ...
    for i in range(1, len(layers)):
        ...
        probe = tar(
            manifest_json(
                layers=[layer["digest"] + ".tar" for layer in manifest["layers"][:i]]
            ),
    ...
    load = tar(
        ...
        *[
            ...
            for layer in layers[start_from:]
        ],
        manifest_json(
            layers=[layer["digest"] + ".tar" for layer in manifest["layers"]]
        ),
    ...
    assert (
        load_res[0].id == manifest["config"]["digest"]
    ), "final image id should match manifest %s == %s" % (
        load_res[0].id,
